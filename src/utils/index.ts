 ...
 *  */
function resolvePathToRealObjectWithArray(
  path: string,
  obj: any,
  addValue?: any
) {
  ...
  const value = path
    .split(".")
    .reduce((prev, curr, currenIndex, initalArray) => {
      if (curr === "[]") {
        ...
        if (Array.isArray(prev)) {
          const arrayWithoutNullElement = prev.filter(
            (el) => el[curr] || ![null, undefined].includes(el[curr])
          );
          const newArray = arrayWithoutNullElement.map((el) => el[curr]);
          ...
          if (
            currenIndex === initalArray.length - 1 &&
            addValue !== undefined
          ) {
