...
async function fetchImageDataFromGit(url: string, githubToken: string, branch: string){
  // https://api.github.com/repos/NJG-connect/landingpageCMS/contents/src/data/services.json?ref=myCMS

  const AllFormatImage = [".jpg",".jpeg",".svg", ".png", ".tif", ".tiff", ".bmp", ".gif"];
  try {
    const reponse = await fetch(
      BASE_URL+url,
      {
    ...
    if (reponse.ok) {
      ...
      await asyncForEach(dataJson, async(element: any)=>{
        if(element.type === "file" && AllFormatImage.some( el => element.name.endsWith(el))){
          const url = await getBlobFile(element.sha, githubToken, element.name.split(".")[1], branch)
...
async function getBlobFile(sha:string , token:string, type:string, branch:string){
  const reponseJson = await fetch(`https://api.github.com/repos/NJG-connect/landingpageWithOwnCMS/git/blobs/${sha}?branch=${branch}`, {
    method: "GET",
    headers: {
      Accept: "application/vnd.github.v3+json",
...
async function postFileOnGit(url: string, file:{
  name: string;
content: string;
branch: string;
}, githubToken: string){
  ...
  try {
    ...
    if (reponseJson.ok){
      return {succes: true, data: {
        name: data.content.name, url: data.content.download_url ,sha: data.content.sha
      }
    }
  }
...
export { fetchJsonDataFromGit, fetchImageDataFromGit, updateJsonDataOnGit, postFileOnGit };